...
import {IconAdjustments, IconCircleXFilled, IconPremiumRights, IconTrees, IconUserDollar} from '@tabler/icons-vue';
...
const getResults = async (page = 1, rowsPerPage = 12) => {
    ...
    try {
        let url = `/${props.strategy}_strategy/getPammMasters?page=${page}&limit=${rowsPerPage}&pamm_type=${props.pammType}`;
                ...
                >
                    <template #content>
                        <div class="flex flex-col items-center gap-4 self-stretch">
                            ...
                            <!-- Performance Section -->
                            <div
                                class="py-2 flex justify-center items-center gap-2 self-stretch border-y border-solid border-gray-200 dark:border-gray-700">
...
<!--                                    </div>-->
                                    ...
                                    <div v-if="pammType === 'ESG'" class="py-1 flex items-center gap-3 self-stretch text-gray-500">
                ...
                >
                    <template #content>
                        <div class="flex flex-col items-center gap-4 self-stretch">
                            ...
                            <!-- Performance Section -->
                            <div
                                class="py-2 flex justify-center items-start gap-2 self-stretch border-y border-solid border-gray-200 dark:border-gray-700">
                                <div class="min-w-16 sm:w-full flex flex-col items-center">
                                    <div class="self-stretch text-gray-950 dark:text-white text-center font-semibold">
                                        {{ formatAmount(master.roi_period, 0) }}
                                    </div>
                                    <div class="self-stretch text-gray-500 text-center text-xs">
                                        {{ $t('public.settlement') }} ({{ $t('public.days') }})
                                    </div>
                                </div>
                                <div class="min-w-[120px] sm:w-full flex flex-col items-center">
                                    <div class="self-stretch text-gray-950 dark:text-white text-center font-semibold">
...
<!--                                    </div>-->
                                    <div class="py-1 flex items-center gap-3 self-stretch text-gray-500">
                                        <IconPremiumRights size="20" stroke-width="1.25"/>
                                        <div class="text-gray-950 dark:text-white text-sm font-medium">
                                            <span class="text-primary-500">$ {{ formatAmount(Number(master.active_pamm_sum_subscription_amount ?? 0) + Number(master.extra_fund)) }}</span> {{ $t('public.fund') }}
                                        </div>
                                    </div>
                                    <div
                                        v-if="pammType === 'ESG'"
                                        class="py-1 flex items-center gap-3 self-stretch text-gray-500"
                                    >
                                        <IconTrees size="20" stroke-width="1.25"/>
                                        <div class="text-gray-950 dark:text-white text-sm font-medium">
                                            <span class="text-primary-500">{{
                                                    formatAmount((master.min_join_equity ?? 0) / 1000, 0)
