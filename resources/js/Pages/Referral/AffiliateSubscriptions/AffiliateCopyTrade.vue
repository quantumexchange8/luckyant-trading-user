...
const loadLazyData = (event) => {
    ...
    try {
        setTimeout(async () => {
            ...
            const url = route('referral.getAffiliateCopyTrade', params);
...
const getMastersByType = async () => {
    ...
    try {
        const response = await axios.get(`/getMastersByType?type=${props.type}`);
...
watch([filters.value['leader_id'], filters.value['master_id']], () => {
    loadLazyData()
});
        ...
        <template #content>
            <div class="w-full">
                <DataTable
                    ...
                    :globalFilterFields="['username', 'email', 'meta_login', 'master_meta_login']"
                >
                    <template #header>
                    ...
                    <template v-if="affiliates?.length > 0">
                        ...
                        >
                            <template #body="{ data }">
                                {{ dayjs(data.approval_date).format('YYYY-MM-DD') }}
                            </template>
                        </Column>
                        ...
                        >
                            <template #body="{ data }">
                                ${{ formatAmount(data.meta_balance) }}
                            </template>
                        </Column>
