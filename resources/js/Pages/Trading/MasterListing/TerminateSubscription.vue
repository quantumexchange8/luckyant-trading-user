...
<template>
    <div class="flex flex-col items-center self-stretch gap-5 md:gap-8">
            ...
            <div class="flex flex-col items-start gap-3 w-full pt-4 self-stretch">
                <div class="flex flex-col gap-1 items-center self-stretch">
                    ...
                    <div class="flex py-1 gap-3 items-start self-stretch">
                        ...
                        <span
                            class="w-full text-error-500 font-semibold text-sm">$ {{ formatAmount(subscriber.total_management_fee) }}</span>
                    </div>
                    <div class="flex py-1 gap-3 items-start self-stretch">
                        <span class="w-full text-gray-500 font-medium text-xs">{{ $t('public.return_amount') }}</span>
                        <span
                            class="w-full text-success-500 font-semibold text-sm">$ {{ formatAmount(subscription.meta_balance - Number(subscriber.total_management_fee)) }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="text-gray-600 dark:text-gray-400 text-xs">
            {{$t('public.confirm_terminate_warning_1')}} <strong>{{ subscription.meta_login }}</strong> {{$t('public.confirm_terminate_warning_2')}}
        </div>
