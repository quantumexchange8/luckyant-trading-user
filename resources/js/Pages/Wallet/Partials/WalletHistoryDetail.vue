        ...
        <div class="p-5 bg-gray-100 dark:bg-gray-600 rounded-lg">
            <div class="flex flex-col items-start gap-3 self-stretch">
                ...
                <div class="flex items-center justify-between gap-2 self-stretch">
                    ...
                    <div class="text-sm sm:text-base text-gray-800 dark:text-white font-semibold">
                        {{ $t('public.' + walletHistory.transaction_type.replace(/([A-Z])/g, '_$1').toLowerCase().replace(/^_/, '')) }}
                    </div>
                </div>
                <div class="flex items-center justify-between gap-2 self-stretch">
                    <div class="font-semibold text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                ...
                <div class="flex items-center justify-between gap-2 self-stretch" v-if="(walletHistory.to_wallet || walletHistory.to_meta_login || walletHistory.payment_account) && walletHistory.transaction_type !== 'WalletAdjustment'">
                    ...
                    <div class="text-sm sm:text-base font-semibold">
                        <span class="text-sm sm:text-base text-gray-800 dark:text-white font-semibold" v-if="walletHistory.transaction_type == 'Withdrawal'">    {{ walletHistory.payment_account ? (walletHistory.payment_account.payment_account_name + ' - ' + walletHistory.payment_account.account_no) : $t('public.' + walletHistory.to_wallet.type) }}</span>
